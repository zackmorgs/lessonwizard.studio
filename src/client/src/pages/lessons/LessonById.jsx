    ...
    getLessonById(id)
      .then((lessonData) => {
        ...
        if (lessonData.studentId) {
          fetches.push(
            getStudentById(lessonData.studentId)
              .then(setStudent)
              .catch(() => null),
          );
        }
        if (lessonData.songIds?.length > 0) {
          fetches.push(
            Promise.all(
              ...
              const map = {};
              entries.forEach(([sid, song]) => {
                if (song) map[sid] = song;
              });
              setSongs(map);
            }),
          );
          ...
          {documents.length > 0 && (
            <ul className="flex flex-col gap-1">
              {documents.map((doc) => (
                <li
                  key={doc.id}
                  className="flex items-center justify-between gap-2 px-3 py-2 bg-white rounded"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    height="24px"
                  ...
                  {doc.pdfUrl && (
                    <a
                      href={`/${doc.pdfUrl}`}
                      download
                      className="text-gray-400 hover:text-blue-500 shrink-0 mr-4"
                      title="Download PDF"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
